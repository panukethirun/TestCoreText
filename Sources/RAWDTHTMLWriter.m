...
//  ASKBook
//
//  Created by Developer Meb on 9/11/18.
//

#import "RAWDTHTMLWriter.h"
//#import "NSString+HTML.h"
#import <Foundation/Foundation.h>
#import "DTCoreText.h"

@interface RAWDTHTMLWriter ()

@end

@implementation RAWDTHTMLWriter
{
...
- (void)_buildOutputAsHTMLFragment:(BOOL)fragment
{
    ...
    for (NSUInteger i=0; i<[paragraphs count]; i++)
    {
        ...
        [_attributedString enumerateAttributesInRange:paragraphRange options:0 usingBlock:^(NSDictionary *attributes, NSRange spanRange, BOOL *stopEnumerateAttributes) {
            ...
            if (underline)
            {
                DTCoreTextFontDescriptor *desc = [attributes fontDescriptor];
                if (desc.symbolicTraits == 0) {
                }
                else if (desc.symbolicTraits == 1) { //italic
                    [retString appendString:@"<em>"];
                }
                else if ([attributes isBold]) { //bold {
                    [retString appendString:@"<strong>"];
                }
                else if (desc.symbolicTraits == 3) { //bold and italic {
                    [retString appendString:@"<strong>"];
                    [retString appendString:@"<em>"];
                }
                [retString appendString:@"<u>"];
